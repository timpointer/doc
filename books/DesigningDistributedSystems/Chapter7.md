...
### 写偏差特征
可以认为写偏差是lost update的通用形式。写偏差发生在两个事务读相同的对象，写不同的对象。特殊场景是读相同的对象，写相同的对象，这时就变成了lost update。
...
### 总结顺序执行
在特定限制下，顺序执行事务是一个可行的顺序隔离方案
- 每个事务必须小而快，因为它一个慢事务会堵塞所有进程。
- 事务数据必须内全部放进内存。读磁盘会拖累事务。
- 写吞吐量受限于单核，或数据被分片后，而不进入夸分片协调。
- 夸分片事务也可行，但性能会被严重受限。
...
锁分为分享锁和互斥锁。
当事务只有读操作时，多个事务都可以获取分享锁，但一旦有写操作发生时，就必须获取互斥锁。
之所以成为两阶段锁，是因为在事务开启的时候获取一次锁，如果途中加入了写操作，会从新去拿互斥所，如果无法获取到，事务会被中断。这就是两阶段锁的由来。

### 两阶段锁的性能
两阶段锁，如果有事务执行时间过长，会导致事务堆积。所以会有不稳定的延迟，这种不稳定对于可靠性应用来说是一个问题。

虽然死锁在read commited级别也会发生，但在2PL中更为频繁。

### Predicate locks
这个是断言锁，会锁住事务中查询的数据范围。重点是能应用在不存在的对象（幽灵）。如果两阶段锁包含了断言锁，就避免了写偏差和其他竞争问题，变成了顺序隔离。

### Index-range locks
不幸的是，断言锁性能不好：大多数数据库采用index-range locking来替代。
首先为查询范围建立索引，然后开启事务后会根据查询条件锁住索引的某个范围。如果没有合适的索引，最差情况，会锁整个表。

# 序列快照隔离(SSL)
要么采用顺序隔离，忍受比较差的性能。要么采用弱隔离级别，面临各种竞争条件。有没有两全其美的方案？

可能没有，但SSL算法可能比较接近。已经有些数据库采用了它。

## 悲观对比乐观并发控制
两阶段锁是悲观并发控制机制。如果事情可能会出错，等情况都安全了再进行下一步。

顺序执行就是悲观的极端情况。等于每个事务都拿个整个数据库的互斥锁。我们让每个事务都很小能很快执行来缓和它。

相对，SSL是一种乐观并发控制，乐观环境下，事务之间不相互堵塞，只在最后提交的时候，检查依赖的查询条件是否有变，如果发生了变化，则中断重试。

数据库需要记录一些信息，来帮助在提交时判断，之前查询的结果是否有变。

# 总结
事务是一种抽象层，允许应用假装没有并发问题和各种软硬件问题。一大类的错误最后都被收敛到一个简单的事务中断，并且应用只需要简单的重试。
