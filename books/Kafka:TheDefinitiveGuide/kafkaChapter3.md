...
## Kafka Producers
不同的使用场景揭示了多样的需求。
我们开发产生端前，需要创建ProducerRecord
发送消息，分为几个步骤，首先创建ProducerRecord类，然后序列化二进制格式，再选择对应的分片，最后添加到对应的待发送队列中，由另一个线程发送消息到kafka的broker。
...
### 顺序保证
要构建可靠的系统，都需要设置重试，但重试会导致乱序，为了避免这种情况发生，需要将in.flight.requests.per.session设置为1.这样能确保顺序执行。
